...
class TeamOverload(object):
    ...
    def run_courseA(self, reverse_on=False, trust_line=True, senario=False,
        ...
        for i, dir in enumerate([-90-angle_calib, 90+angle_calib, -90-angle_calib]):
            ...
            if trust_line:
                self.trace_line(turndir="None", forwardspd=forwardspd, turnspd=turnspd, motordiff=motordiff if i != 2 else 18)
            else:
                self.trace_line(forwardspd=forwardspd, turnspd=turnspd, motordiff=motordiff if i != 2 else 18)
